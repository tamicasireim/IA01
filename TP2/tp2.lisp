...
(defun recherche (etatA etatR)
  (recherche_a etatA etatR 3 (list etatA)))

(defun recherche_a (etatA etatR profondeur chemin)
  (cond ((equal etatA etatR) chemin)
	((eq profondeur 0) NIL)
	(T (dolist (etat (successeurs etatA) NIL)
	     (if (not (is_member etat chemin))
	       (let ((nouveauchemin (append chemin (list etat))))
		 ;(print nouveauchemin)
		 (let ((research (recherche_a etat etatR (- profondeur 1) nouveauchemin)))
		   (if (not (equal research NIL))
		     (return-from recherche_a research))))
	       ))
	   (dolist (etat (successeurs etatA) NIL)
	     (if (not (is_member etat chemin))
	       (let ((nouveauchemin (append chemin (list etat))))
		 ;(print nouveauchemin)
		 (let ((research (recherche_a etat etatR profondeur nouveauchemin)))
		   (if (not (equal research NIL))
		     (return-from recherche_a research))))
		 ))
	   )))
